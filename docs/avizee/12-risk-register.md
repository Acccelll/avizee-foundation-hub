...
| RK-15 | **Credencial SMTP exposta em texto claro** no código-fonte atual (`send_email.php`) e trafegada por canal comum | Alta | Alto | Revogar imediatamente a senha de aplicativo na conta Google (O-27); na v1 usar segredo de servidor, nunca literal em código; nenhum segredo em repositório | **Ativo — ação do usuário** |
