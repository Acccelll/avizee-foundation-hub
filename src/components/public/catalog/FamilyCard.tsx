  ...
  return (
    <article className="relative flex h-full flex-col overflow-hidden rounded-[12px] border border-border bg-background transition hover:border-emphasis">
      <div className="relative aspect-4/3 bg-surface">
        {family.image.is_placeholder ? (
          <div className="flex h-full flex-col items-center justify-center gap-2 text-text-muted">
            ...
            <span className="px-4 text-center text-[12px]">{family.image.alt}</span>
          </div>
        ) : (
          <img
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        {family.summary && (
          <p className="line-clamp-3 text-[15px] text-text-secondary">{family.summary}</p>
        )}
        ...
        <div className="mt-auto flex flex-wrap items-center gap-2 pt-2">
          <span className="rounded-full border border-border-subtle px-3 py-1 text-[12px] text-text-secondary">
            {variacoes}
          </span>
          {family.applications.slice(0, 2).map((app) => (
            <span
              key={app}
              className="rounded-full border border-border-subtle px-3 py-1 text-[12px] text-text-secondary"
            >
              {app}
            </span>
          ))}
